...
export default function PeopleDetails({
  ...
  const saveUser = async () => {
    ...
    const updatedUser = { ...user, email, role, firstName, lastName };
  ...
  return (
    <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
      ...
      {!editing && <span className="wd-roles"> {user.role} </span>} <br />
      <b>Email:</b>{" "}
      {editing && (
        <input
          type="email"
      ...
      {!editing && <span className="wd-roles"> {user.email} </span>} <br />
